...
def loadSubjects(filename):
    ...
    for line in inputFile:
        ...
        courses[name] = (int(value), int(work))
...
def greedyAdvisor(subjects, maxWork, comparator):
    ...
    for i in range(len(keyList)-1):
        ...
        if(comparator(subjects[keyList[i]], subjects[keyList[i+1]])) == True:
                
            if subjects[keyList[i]][WORK] + totalWork < 15:
                greedyDict[keyList[i]] = subjects[keyList[i]]
                totalWork += subjects[keyList[i]][WORK]
