...
class Hand(object):
    ...
    def update(self, word):
        ...
        for x in word:
            ...
            for letter, freq in list(self.handDict.items()):
          
                if letter == x and freq == 1:
                    del self.handDict[letter]                    
                elif letter == x:
                    self.handDict[letter] = (freq-1)
    ...
    def containsLetters(self, letters):
        ...
        False otherwise
        """
        letterDict = getFrequencyDict(letters)
        
        for k, v in letterDict.items():
            if self.handDict.get(k, -1) >= v:
                return True
            else:
                return False
    ...
    def __eq__(self, other):
        ...
        """
        otherDict = getFrequencyDict(other)
        for k,v in otherDict.items():
            if self.handDict.get(k,object()) != v:
                return False
